 ...
 *
 */
public class GETorFILTERstartsWithAny implements XMLElement,  XMLFilter, StringProcessor {
    ...
    /**
     * @return the filter that checks if the text in the XMLElement starts with any of the terms
	...
	return new XMLElementFilter() {
		...
		for (int i = 0; i < stringChildList.size();i++)
		    terms[i] = stringChildList.get(i).getText();
		if (caseSensitive) {
		    boolean returnvalue = (StringHelper.startsWithAny(element.getText(),terms));
		    if (include)
			return returnvalue;
		    else 
			return !returnvalue;
		    ...
		    boolean returnvalue = (StringHelper.startsWithAnyIgnoreCase(element.getText(),terms));
		    if (include)
			return returnvalue;
		    else 
			return !returnvalue;
